...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isSignUp && !whatsappNumber.trim()) {
      setErrorMessage('Please enter your WhatsApp mobile number for AI payment verification.');
      return;
    }
    ...
    try {
      if (isSignUp) {
        ...
        if (newUser) {
          await UserProfileService.getOrCreateUserProfile(newUser, { whatsappNumber });
        }
        setSuccessMessage('Account created & WhatsApp registered successfully! Redirecting...');
      } else {
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5">
          {isSignUp && (
            <>
              ...
              <div>
                <label className="text-xs font-bold text-slate-700 block mb-1">WhatsApp Mobile Number (For AI OCR Payments) *</label>
                <div className="relative">
